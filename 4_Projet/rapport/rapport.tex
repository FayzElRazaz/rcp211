...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/cart-pole.png}
    \caption{Environnement CartPole utilisé pour les expériences d'apprentissage par renforcement et d'apprentissage par récompense inverse. L'objectif consiste à maintenir le pendule en équilibre en appliquant des forces horizontales au chariot.}
    \label{fig:cartpole}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/ppo_learned_rewards.png}
...
% \midrule
% Récompense linéaire & 27.65 & 13.64 \\
% Récompense quadratique & 8.95 & 0.70 \\
% Réseau de neurones & 9.37 & 0.966 \\
% \bottomrule
% \end{tabular}
% \caption{Performance des agents PPO entraînés avec les récompenses apprises par discrimination état par état.}
% \end{table}
...
Après deux itérations, les résultats obtenus sont les suivants :
\begin{itemize}
    \item longueur moyenne des épisodes : \(500.0 \pm 0.0\) ;
    \item score moyen selon la récompense native de \texttt{CartPole}, utilisée uniquement pour analyse : \(500.0 \pm 0.0\).
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/preference_trajectory_lengths.png}
    \caption{Evolution de la longueur des trajectoires préférées}
\end{figure}
...
Récompense linéaire & 27.65 & 13.64 \\
Récompense quadratique & 8.95 & 0.70 \\
Récompense par réseau de neurones & 9.37 & 0.966 \\
Reward par préférence de trajectoires & 500.0 & 0.0 \\
\bottomrule
\end{tabular}
...
Le nouvel agent atteint un score moyen de :
\[
499.8 \pm 0.87
\]

selon la récompense native de \texttt{CartPole}, utilisée uniquement comme métrique d’évaluation.

Le nouvel agent entraîné uniquement à partir de la récompense apprise atteint un score moyen de 499.8, soit quasiment la performance maximale observée sur CartPole. Ce résultat indique que la fonction de récompense apprise contient suffisamment d'information pour reproduire un comportement expert sans accès aux démonstrations initiales. Bien que cette conclusion reste limitée au cadre de l'environnement étudié, elle constitue un résultat encourageant quant à la qualité de la récompense apprise.
...
L'objectif de ce projet était d'étudier différentes approches d'apprentissage par récompense inverse sur l'environnement CartPole afin de déterminer dans quelle mesure il est possible de reconstruire une fonction de récompense à partir d'observations de comportements experts.

Une première approche reposant sur l'apprentissage supervisé d'une fonction discriminante entre états experts et non experts a été étudiée. Trois paramétrisations ont été testées : linéaire, quadratique et réseau de neurones. Bien que ces modèles soient capables de distinguer partiellement les démonstrations expertes des démonstrations aléatoires, les récompenses obtenues se sont révélées insuffisantes pour guider efficacement un nouvel apprentissage par renforcement. Ces résultats illustrent une limite importante des approches purement discriminatives : une bonne séparation entre classes n'implique pas nécessairement l'obtention d'une récompense exploitable par un agent.

Une seconde approche fondée sur l'apprentissage par préférences a ensuite été mise en œuvre. À chaque itération, des trajectoires ont été comparées en fonction de leur qualité, permettant d'entraîner progressivement un modèle de récompense capable de reproduire ces préférences. Les résultats obtenus montrent une amélioration continue des trajectoires générées jusqu'à atteindre la performance maximale de l'environnement CartPole.

Enfin, la récompense apprise a été sauvegardée puis réutilisée indépendamment des démonstrations initiales afin d'entraîner un nouvel agent PPO depuis zéro. Cet agent atteint à son tour une performance quasi optimale, avec un score moyen de 499.8 sur 500. Ce résultat constitue un élément particulièrement important puisqu'il montre que la récompense apprise ne se contente pas de reproduire les préférences observées : elle capture effectivement les caractéristiques essentielles du comportement expert et peut être réutilisée comme signal d'apprentissage autonome.

Ainsi, dans le cadre de CartPole, ce projet met en évidence l'intérêt des approches basées sur les préférences pour l'apprentissage de fonctions de récompense. Alors que les approches discriminatives étudiées n'ont pas permis de reconstruire une récompense exploitable, l'apprentissage par préférences a conduit à une récompense suffisamment informative pour permettre l'émergence d'un comportement expert, démontrant ainsi la faisabilité pratique de l'apprentissage par récompense inverse sur ce problème.
